...
- (id)initWithFrame:(CGRect)frame viewType:(int)type{
    ...
    if (self) {
        ...
        //  初始化箭头视图
        arrowView = [[UIImageView alloc] initWithFrame:CGRectMake((k_STATE_VIEW_INDICATE_WIDTH - 32) / 2, (k_STATE_VIEW_HEIGHT - 32) / 2, 16, 28)];
...
//            stateLabel.text = viewType == k_VIEW_TYPE_HEADER ? stateLabel.text : @"已加载全部数据";
            stateLabel.text = viewType == k_VIEW_TYPE_HEADER ? @"ffff" : @"没有更多内容";
            arrowView.hidden = YES;
            break;
...
//        [footerView setBackgroundColor:GrayColor];
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        headerView = [[StateView alloc] initWithFrame:CGRectMake(0, -40, frame.size.width, 40) viewType:k_VIEW_TYPE_HEADER];
        footerView = [[StateView alloc] initWithFrame:CGRectMake(0, self.tableFooterView.frame.origin.y, frame.size.width, frame.size.height) viewType:k_VIEW_TYPE_FOOTER];
//        [footerView setBackgroundColor:GrayColor];
