...
- (id)initWithView:(UIView *)view {
	...
	id me = nil;
//    id me = [self initWithFrame:CGRectMake(0, NavBar_Height, Screen_Width, view.bounds.size.height-NavBar_Height)];
	// We need to take care of rotation ourselfs if we're adding the HUD to a window
	if ([view isKindOfClass:[UIWindow class]]) {
        me = [self initWithFrame:view.bounds];
		[self setTransformForCurrentOrientation:NO];
	}
    else
    {
        me = [self initWithFrame:CGRectMake(0, NavBarHeight, ScreenWidth, view.bounds.size.height-NavBarHeight)];
    }
	return me;
}
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        ...
        PINFO(@"targetForExecution", targetForExecution);
//        PINT(@"methodForExecution", methodForExecution);
        PINFO(@"objectForExecution", objectForExecution);
        
#pragma clang diagnostic pop
		// Task completed, update view in main thread (note: view operations should
		// be done only in the main thread)
        /********Add by zhangfeng 2012-11-29*************/
...
- (void)setupLabels {
    ...
    self.detailsLabelFont = [UIFont systemFontOfSize:13];
...
//        self.frame = CGRectMake(0, Navbar_Height, Screen_Width, parent.bounds.size.height-Navbar_Height);
	}
    ...
    labelSize.height = label.frame.size.height;
...
//    CGFloat height = totalSize.height - label.height;
//    [label sizeToFit];
//    height += label.height;
